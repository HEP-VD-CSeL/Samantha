...
async def detect(ws, workspace, file, classes):
  ...
  while True:
    ...
    try:
      ...
      print(f"Client disconnected, stopping detection. Error: {e}")
      cap.release()
...
async def anonymize(ws, workspace, file, detections_list):
  ...
  while True:
    ...
    for detection in detections:
      if not detection['blur']:
        continue

      # Get bounding box
      x1, y1, x2, y2 = map(int, [detection['positions']['x1'],detection['positions']['y1'], detection['positions']['x2'], detection['positions']['y2']])
